...
def point_to(fresh: Path):
    app.GAME_DIR = fresh.resolve()
    app.MODS_DIR = app.GAME_DIR / "mods"
    app.LOAD_ORDER_FILE = app.MODS_DIR / "mod_load_order.txt"
    app.CONFIG_FILE = fresh.resolve() / "config_test.json"   # 防污染项目根 config.json


def all_payload_files():
    """payload 内全部待释放文件（相对路径）"""
    out = []
    for sub in app.DMF_SUBTREES:
        base = app.DMF_PAYLOAD_DIR / sub
        if base.is_dir():
            for f in base.rglob("*"):
                if f.is_file():
                    out.append(f.relative_to(app.DMF_PAYLOAD_DIR))
    return out
...
check("缺失组件数 = 7（旧 mod_manager.lua + .off 禁用不算缺失）", len(st["missing"]) == len(app.DMF_FILES) - 2, str(st["missing"]))
...
for rel in app.DMF_FILES:
    check(f"关键文件到位: {rel}", (app.GAME_DIR / rel).is_file())
# 内容与 payload 一致
import hashlib
ok_hash = all(
    hashlib.sha256((app.DMF_PAYLOAD_DIR / x).read_bytes()).digest()
    == hashlib.sha256((app.GAME_DIR / x).read_bytes()).digest()
...
# 自动装载恢复
check(".off 残留已清除", not (app.GAME_DIR / app.AUTOPATCH_DLL_OFF).exists())
...
point_to(FRESH)
# 假数据不含 patch_999 字样
(app.GAME_DIR / "bundle" / "bundle_database.data").write_bytes(b"FAKE_DB_BINARY_DATA")
r3 = app.api_dmf_install()
check("文件已释放（即使补丁失败）", all((app.GAME_DIR / rel).is_file() for rel in app.DMF_FILES))
