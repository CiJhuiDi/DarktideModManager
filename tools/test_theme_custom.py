...
async def main():
    ...
    r = await app.api_theme_custom_upload(mode='light', file=uf)
    ...
    r2 = await app.api_theme_custom_upload(mode='dark', file=uf2)
    ...
    r3 = await app.api_theme_custom_upload(mode='neon', file=uf3)
    ...
    r4 = await app.api_theme_custom_upload(mode='dark', file=uf4)
    test('覆盖上传 ok', r4.get('ok'), r4)
    files = list(app.CUSTOM_THEME_DIR.glob('bg.*'))
