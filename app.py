...
def scan_mods() -> list:
    ...
    for i, n in enumerate(enabled):
        if n not in seen:
            result.append({"name": n, "display_name": "", "version": "", "enabled": True, "order": i, "missing": True})
...
def import_mod_archive(filename: str, data: bytes) -> dict:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if not mod_files:
            return {"file": filename, "ok": False,
                    "error": "压缩包内没有 .mod 文件，不是 DMF mod 包（整个整合包请用「导入整合包」）"}
        first = mod_files[0]
...
async def api_import_mods(files: list[UploadFile] = File(...)):
    if not is_valid_game_dir(GAME_DIR):
    ...
    for f in files:
        try:
            ...
            continue
        results.append(import_mod_archive(f.filename or "mod.zip", data))
...
def api_dmf_install():
    """一键安装 DMF：释放内置组件（已有文件先备份）→ 恢复自动装载 → 打补丁激活 mods"""
    ...
    # 3. 打补丁激活 mods
    msg = f"✓ DMF 安装完成（{len(installed)} 个组件）"
    if installed:
        r = _run_patch("--patch")
        if r.get("patched"):
            msg += "，补丁已激活，mods 已就绪"
...
def is_pack_like(out: Path) -> bool:
    ...
    if mods_dir.is_dir():
        ...
        if (root / "bundle").is_dir() and list((root / "bundle").glob("*.patch_999")):
            return True
        # 只有 1 个 mod 文件夹 → 单 mod 的 mods/ 包裹，不是整合包（除非带启停清单）
...
@app.get("/api/backups")
def api_backups():
    """列出归档备份：整合包归档 pack_backup_* 与 DMF 组件备份 dmf_backup_*"""
    ...
    for d in sorted(BACKUP_DIR.iterdir(), reverse=True):
        ...
        if d.name.startswith("pack_backup_"):
            ...
            if mods_dir.is_dir():
                ...
                backups.append({
                    "id": d.name, "type": "pack",
                    "created": d.name[len("pack_backup_"):],
                    "mods": mods, "count": len(mods),
                    "has_load_order": (mods_dir / "mod_load_order.txt").is_file(),
                })
        elif d.name.startswith("dmf_backup_"):
            files = [x for x in d.rglob("*") if x.is_file()]
            backups.append({
                "id": d.name, "type": "dmf",
                "created": d.name[len("dmf_backup_"):],
                "count": len(files),
            })
...
def api_backup_restore(bid: str):
    """恢复整合包归档：当前 mods 先归档，再把备份的 mods 恢复回去"""
    if not is_valid_game_dir(GAME_DIR):
...
def api_backup_delete(bid: str):
    """删除归档备份"""
    d = BACKUP_DIR / bid
    if not d.is_dir() or not (bid.startswith("pack_backup_") or bid.startswith("dmf_backup_")):
